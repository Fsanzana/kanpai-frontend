class MangaReader {
  ...
  chapterSelector(chapter) {
    if (parseInt(this.currentChapter) !== parseInt(chapter)) {
      if (!this.overviewMode) {
  ...
  setGui() {
    ...
    var temp =
      ...
      '<button id="prevChapter" onclick="reader.prevChapterButton()" title="Capitulo previo" style="position: absolute;left: 20%;font-size: 1rem;width: 8rem;height: 2.35rem;margin-top: -0.7rem;color: #cacaca;border: 0.1rem solid #303030;"><- PREVIO</button>' +
      '<button id="nextChapter" onclick="reader.nextChapterButton()" title="Próximo capitulo" style="position: absolute;right: 20%;font-size: 1rem;width: 8rem;height: 2.35rem;margin-top: -0.7rem;color: #cacaca;border: 0.1rem solid #303030;">SIGUIENTE -></button></div>' +
      //ZOOM
